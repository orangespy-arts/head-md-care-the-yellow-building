...
0611
    现在要来描述一下架构了
    一共有四个state
    state1：screensaver 模式 相机跟随猫跳跃不同的窗台
    state2：九宫格模式 用户点击screen saver 到这个模式 然后可以和这九宫格 九个房间的东西互动 相机固定 猫照常跳
    state3：当交互到一定时间 或者 所有交互都点完了（我不知道该选那个）九个房间里的东西开始消失 最后剩下空房间们 猫也消失
    state4：zoom out 到一个更远的地方 就是另一栋楼里的一个房间看向这个房价的视角，然后猫出现在这里，最终从这个房间里看到外面变黑 结束体验
